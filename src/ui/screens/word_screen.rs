...
use crate::ui::elements::word_area::WordArea;
...
impl WordScreen {
    ...
    pub fn new(w: u16, h: u16, screen_settings: &ScreenSettings) -> WordScreen {
    ...
    fn create_layout(w: u16, h: u16, data_area_width: u16, show_info_bar: bool, show_offset_bar: bool, show_location_bar: bool, location_bar_width: u16) -> WordScreen {
        ...
        let max_width = max_aviable_width / 7;
        let wa_width = std::cmp::min(data_area_width, max_width) * 5;
        let ta_width = std::cmp::min(data_area_width, max_width) * 2;
        let wa = WordArea::new(ob_width + ls.width(), y0, wa_width, new_h);

        let cs_width = 5 * (max_aviable_width - wa_width - ta_width) / 7;
        let cs = Separator::new(wa.x0() + wa.width(), y0, cs_width, new_h);
        let ta = TextArea::new(cs.x0() + cs.width(), y0, ta_width, new_h);

        let rs_width = w - ob_width - ls.width() - wa_width - cs_width - ta_width - lb_width;
...
impl Screen for WordScreen {
    ...
    fn inc_row_size(&mut self) {
        if self.word_area.row_size() < 2 * self.max_data_width {
            *self = Self::create_layout(self.w, self.h, self.word_area.row_size()/ 2 + 1 , self.show_info_bar, self.show_offset_bar, self.show_location_bar, self.location_bar.width());
        }
    }

    fn dec_row_size(&mut self) {
        if self.word_area.row_size() > 2 {
            *self = Self::create_layout(self.w, self.h, self.word_area.row_size() / 2 - 1 , self.show_info_bar, self.show_offset_bar, self.show_location_bar, self.location_bar.width());
        }
    }

    fn toggle_offset_bar(&mut self) {
        let data_width = if self.word_area.row_size() == 2 * self.max_data_width { u16::MAX } else { self.word_area.row_size() / 2 };
    ...
    fn show_location_bar(&mut self, value: bool) {
        if value != self.show_location_bar {
            *self = Self::create_layout(self.w, self.h, self.word_area.row_size()/2, self.show_info_bar, self.show_offset_bar, value, self.location_bar.width());
        }
    }
    ...
    fn is_over_data_area(&self, col: u16, row: u16) -> bool {
        self.text_area.contains_position(col, row) || self.word_area.contains_position(col, row)
    }
    ...
    fn screen_coord_to_file_offset(&self, init_offset: usize, column: u16, row: u16) -> Option<usize> {
        if let Some((loc_col, loc_row)) = self.word_area.to_local_position(column, row) {
            let bytes_per_row = self.word_area.row_size() as usize;
