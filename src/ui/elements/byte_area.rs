...
impl ByteArea {
    ...
    pub fn new(x: u16, y: u16, w: u16, h: u16) -> ByteArea {
        ByteArea {
            x, y, w, h,
            bytes_per_row: (w+1)/3,
        }
        ...
        for y in self.y..self.y + self.h {
            ...
            for x in 0..self.bytes_per_row {
                ...
                let (new_fg_color, new_bg_color) = if cursor_state.is_visible() && cursor_state.position() == offset + counter {
                    ...
                    } else if let Some(color) = file_buffer.highlight_color(offset + counter) {
                        color
